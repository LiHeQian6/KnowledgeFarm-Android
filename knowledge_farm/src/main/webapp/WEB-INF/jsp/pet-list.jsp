...
<head>
    ...
    <script>
        ...
        window.onload = function(){
            ...
            $("#initPetManager1").attr("class","current");
        }

        //删除单个作物信息
        function deleteOnePet(id){
            layer.confirm('确认要删除吗？',function(index){
                $.post("${ctx}/admin/pet/deleteOnePet",{"id":id},function(data){
                    if(data == "succeed"){
                        window.location.href="${ctx}/admin/pet/findPetPage?name=${param.name}&&pageNumber=${petPage.currentPageNum}&&pageSize=${petPage.pageSize}&&exist=1";
                    }else if(data == "fail"){
                        layer.msg('删除失败');
                    }
                })
        ...
        function deleteMultiPet() {
            ...
            layer.confirm('确认要批量删除吗？',function(index){
                if(deleteStr != ""){
                    $.post("${ctx}/admin/pet/deleteMultiPet",{"deleteStr":deleteStr},function(data){
                        if(data == "succeed"){
                            window.location.href="${ctx}/admin/pet/findPetPage?name=${param.name}&&pageNumber=${petPage.currentPageNum}&&pageSize=${petPage.pageSize}&&exist=1";
                        }else if(data == "fail"){
                            layer.msg('删除失败');
        ...
        //添加作物信息
        function addPet(title,url,w,h){
            x_admin_show(title,url,w,h);
        }

        //修改作物信息
        function updatePet(title,url,w,h) {
            x_admin_show(title,url,w,h);
        }
...
<div class="wrapper">
    ...
    <div class="page-content">
        <div class="content">
            ...
            <xblock>
                <button class="layui-btn layui-btn-danger" onclick="deleteMultiPet()">
                    <i class="layui-icon">&#xe640;</i>批量删除
                </button>
                <button class="layui-btn" onclick="addPet('添加宠物','${ctx}/admin/pet/toAdd','600','500')">
                    <i class="layui-icon">&#xe608;</i>添加
                </button>
                <a href="${ctx}/admin/pet/findPetPage?name=${param.name}&&pageNumber=${petPage.currentPageNum}&&pageSize=${petPage.pageSize}&&exist=1">
                ...
                <c:forEach var="petPage" items="${petPage.list}">
                    <tr>
                        ...
                        <td class="td-manage" align="center">
                            <a style="text-decoration:none"  onclick="updatePet('编辑','${ctx}/admin/pet/toEdit?id=${petPage.id}','600','400')" href="javascript:;" title="编辑">
                                <i class="layui-icon">&#xe642;</i>
                            </a>
                            <a title="删除" href="javascript:;" onclick="deleteOnePet(${petPage.id})" style="text-decoration:none">
                                <i class="layui-icon">&#xe640;</i>
                            </a>
                        </td>
                    </tr>
