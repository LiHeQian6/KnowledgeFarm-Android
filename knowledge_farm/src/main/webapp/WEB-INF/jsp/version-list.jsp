...
<head>
    ...
    <script>
        ...
        window.onload = function(){
            $("#initVersionManager").attr("class","sub-menu opened");
            $("#initVersionManager1").attr("class","current");
        };

        function deleteOneVersion(id){
            layer.confirm('确认要删除吗？',function(index){
                $.post("${ctx}/admin/version/deleteOneVersion",{"id":id},function(data){
                    if(data == "succeed"){
                        window.location.href="${ctx}/admin/version/findVersionPage?versionName=${param.versionName}&&pageNumber=${versionPage.currentPageNum}&&pageUtilSize=${versionPage.pageSize}";
                    }else if(data == "fail"){
                        layer.msg('删除失败');
                    }
                })
        ...
        function deleteMultiVersion() {
            ...
            layer.confirm('确认要批量删除吗？',function(index){
                if(deleteStr != ""){
                    $.post("${ctx}/admin/version/deleteMultiVersion",{"deleteStr":deleteStr},function(data){
                        if(data == "succeed"){
                            window.location.href="${ctx}/admin/version/findVersionPage?versionName=${param.versionName}&&pageNumber=${versionPage.currentPageNum}&&pageUtilSize=${versionPage.pageSize}";
                        }else if(data == "fail"){
                            layer.msg('删除失败');
        ...
        //添加作物信息
        function addVersion(title,url,w,h){
...
<div class="wrapper">
    ...
    <div class="page-content">
        <div class="content">
            <!-- 右侧内容框架，更改从这里开始 -->
            <form class="layui-form xbs" action="${ctx}/admin/version/findVersionPage">
                <div class="layui-form-pane" style="text-align: center;">
                    <div class="layui-form-item" style="display: inline-block;">
                        <div class="layui-input-inline">
                            <select name="versionName">
                                <option value=" " selected="">请选择版本名称</option>
                                <c:forEach var="versionName" items="${versionNames}">
                                    <c:if test="${versionName == param.versionName}">
                                        <option value="${versionName}" selected>${versionName}</option>
                                    </c:if>
                                    <c:if test="${versionName != param.versionName}">
            ...
            <xblock>
                <button class="layui-btn layui-btn-danger" onclick="javascript:deleteMultiVersion()">
                    <i class="layui-icon">&#xe640;</i>批量删除
                </button>
                <button class="layui-btn" onclick="addVersion('添加版本','${ctx}/admin/version/toAdd','600','500')">
                    <i class="layui-icon">&#xe608;</i>添加
                </button>
                <a href="${ctx}/admin/version/findVersionPage?versionName=${param.versionName}&&pageNumber=${versionPage.currentPageNum}&&pageSize=${versionPage.pageSize}">
                ...
                <span class="x-right" style="line-height:40px">共有数据：${versionPage.totalCount} 条</span>
            </xblock>
            <table class="layui-table">
                ...
                <tbody align="center">
                <c:forEach var="version" items="${versionPage.list}">
                    <tr>
                        <td><input type="checkbox" value="${version.id}" name="checkBox"></td>
                        <td>${version.id}</td>
                        ...
                        <td>
                            <a href="${ctx}/admin/version/downloadVersion?id=${version.id}">
                        ...
                        <td class="td-manage" align="center">
                            <a title="删除" href="javascript:;" onclick="deleteOneVersion(${version.id})" style="text-decoration:none">
        ...
        <!-- 分页处理开始 -->
        <div align="center">
            <a  class="page" style="margin-left:25px;" href="${ctx}/admin/version/findVersionPage?versionName=${param.versionName}&&pageNumber=1&&pageSize=${versionPage.pageSize}">首页</a>
            <a  class="page" href="${ctx}/admin/version/findVersionPage?versionName=${param.versionName}&&pageNumber=${versionPage.prePageNum}&&pageSize=${versionPage.pageSize}">上一页</a>
            <a  class="page" href="${ctx}/admin/version/findVersionPage?versionName=${param.versionName}&&pageNumber=${versionPage.nextPageNum}&&pageSize=${versionPage.pageSize}">下一页</a>
            <a  class="page" href="${ctx}/admin/version/findVersionPage?versionName=${param.versionName}&&pageNumber=${versionPage.totalPageNum}&&pageSize=${versionPage.pageSize}">末页</a>
        </div>
        <div align="center" style="margin-top:20px;">
            <span style="margin-right:10px;">${versionPage.currentPageNum}</span>
            <span>/</span>
            <span style="margin-left:10px;">${versionPage.totalPageNum}</span>
        </div>
        <!-- 分页处理结束 -->
    </div>
    <!-- 右侧主体结束 -->
